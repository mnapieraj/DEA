...
if(dataTree$errMsg == "") {
  ...
  if(errMsg == "") {
    ...
    source("superEfficiency.R")
	result <- calculateSuperEfficiencyForAll(dmuData, dmuData$transformToUtilities)
	#print(result)
    saveResult(dmuData$altIDs, result[,1], "superEfficiency")
	saveResult(dmuData$altIDs, result[,2], "distance")
