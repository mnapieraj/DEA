...
saveResult <- function (altIDs, res, fileName) {
  cols <- ncol(res)
  colnames(res) <- c(paste("geq", altIDs))
  rownames(res) <- (altIDs)
  setwd(outDirectory)
...
if(dataTree$errMsg == "") {
  ...
  if(errMsg == "") {
    ...
    source("pairwiseComparisonsSMAA-VDEA.R")
	comp <- calculateComparisonsSMAAForAll(dmuData, dmuData$samplesNo, dmuData$transformToUtilities)
	#print(comp)
    saveResult(dmuData$altIDs, comp, "pairwiseOutrankingIndices")
