...
if(dataTree$errMsg == "") {
  ...
  if(errMsg == "") {
	...
	effBounds <- calculateEfficiencyBoundsForAll(dmuData, dmuData$transformToUtilities)
	    saveResult(dmuData$altIDs, effBounds[,1], "minEfficiency")
    saveResult(dmuData$altIDs, effBounds[,2], "maxEfficiency")
    saveResult(dmuData$altIDs, effBounds[,3], "minDistance")
    saveResult(dmuData$altIDs, effBounds[,4], "maxDistance")
